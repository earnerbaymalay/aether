#!/usr/bin/env bash
# Aether-AI v7.5: Professional System Build
...
echo -e "\033[0;34m  NEURAL OPERATING INTERFACE // V 7.5\033[0m"
echo ""
...
CHOICE=$(gum choose --cursor.foreground "$ACCENT" --item.foreground "$WHITE" --selected.foreground "$CYAN" --header "  [ SELECT NEURAL PATH ]" --header.foreground "$DIM" --padding "1" "  ●  HERMES-3  (AGENTIC TOOLS) " "  ●  GEMMA-2   (LOGIC & REASONING) " "  ●  SYSTEM DIAGNOSTICS " "  ●  SHUTDOWN INTERFACE ")
...
case "$CHOICE" in
    ...
    *"DIAGNOSTICS"*)
        printf "Metric,Value\nPower,$BATT%%\nDisk,$STORAGE\nCores,$THREADS\nEngine,Llama-CLI\nStatus,Superior" | \
        gum table --border.foreground "$ACCENT" --cell.foreground "$WHITE" --header.foreground "$ACCENT"
