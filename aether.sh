#!/usr/bin/env bash
# Aether-AI v7.7: Absolute Syntax Fix
...
# 2. LOGO - RAW ANSI PRINTING (Bypasses gum for perfect alignment)
echo -ne "\033[1;34m"
figlet -f small "  A E T H E R"
echo -e "\033[0;34m  NEURAL OPERATING INTERFACE // V 7.7\033[0m"
echo ""

# 3. STATUS BAR (Hyphenated flags work here)
...
case "$CHOICE" in
    *"HERMES-3"*)
        ...
        $BIN -m "$MODELS/hermes-3-8b.gguf" -cnv -t $THREADS -p "You are Aether, an AI agent. Use tools." ;;
    *"GEMMA-2"*)
        gum spin --spinner.foreground "$CYAN" --title "Mounting Gemma-2..." -- sleep 2
        $BIN -m "$MODELS/gemma-2-9b.gguf" -cnv -t $THREADS ;;
    *"DIAGNOSTICS"*)
        # FIXED SYNTAX: Using Dot Notation for Table as requested by your gum error
        printf "Metric,Value\nPower,$BATT%%\nDisk,$STORAGE\nCores,$THREADS\nStatus,Stable" | \
        gum table --border.foreground "$ACCENT" --header.foreground "$ACCENT"
        echo ""
